...
template <class Ctr, class SizeType> inline void reserve_if_possible(Ctr &ctr, const SizeType &n)
{
...
class slot_map
{
    ...
    template <class Integral> static constexpr void set_index(Key &k, Integral value)
    {
        auto &[idx, gen] = k;
...
private:
    ...
    constexpr iterator erase_slot_iter(slot_iterator slot_iter)
    {
        ...
        if (value_iter != value_back_iter) {
            ...
            *reverse_map_iter =
                static_cast<key_index_type>(std::distance(slots_.begin(), slot_back_iter));
        }
        values_.pop_back();
